<x-app-layout>
    ...
    <div class="py-12 bg-slate-50 min-h-screen">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm shadow-slate-200/50 sm:rounded-2xl border border-slate-200">
                <div class="p-8">
                    <form method="POST" action="{{ route('materias.store') }}" x-data="materiaForm()">
                        @csrf
                        ...
                        <div class="mb-10">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div class="md:col-span-2">
                                    ...
                                    <x-text-input id="nombre" class="block mt-1 w-full text-lg font-medium" type="text" name="nombre" :value="old('nombre')" required autofocus placeholder="Ej: Inglés Básico 101" />
                                ...
                                <div>
                                    ...
                                    <select id="nivel" name="nivel" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full text-slate-700" required>
                                        ...
                                        <option value="c1" {{ old('nivel') == 'c1' ? 'selected' : '' }}>C1 - Intermedio Alto</option>
                                        <option value="c2" {{ old('nivel') == 'c2' ? 'selected' : '' }}>C2 - Avanzado</option>
                                    </select>
                                    ...
                                    <select id="profesor_id" name="profesor_id" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full text-slate-700 font-medium bg-slate-50" required>
                                        @if(Auth::user()->role === 'admin')
                                            ...
                                            @foreach($profesores as $profe)
                                                <option value="{{ $profe->id }}" {{ old('profesor_id') == $profe->id ? 'selected' : '' }}>
                                ...
                                <div>
                                    ...
                                    <x-text-input id="fecha_fin" class="block mt-1 w-full text-slate-500" type="date" name="fecha_fin" :value="old('fecha_fin')" />
                                    <p class="mt-1 text-xs text-slate-400">Opcional. Deja en blanco si es curso continuo.</p>
                                </div>

                                <div class="md:col-span-2 pt-2">
                                    <label for="activa" class="inline-flex items-center">
                                        <input id="activa" type="checkbox" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500 w-5 h-5 cursor-pointer" name="activa" value="1" {{ old('activa', true) ? 'checked' : '' }}>
    ...
    <!-- Script Alpine.js para gestionar los Horarios -->
    <script>
        function materiaForm() {
            return {
                horarios: [
                    { id: Date.now(), dia_semana: 'lunes', hora_inicio: '08:00', hora_fin: '10:00' }
                ],
                addHorario() {
                    this.horarios.push({ 
                        id: Date.now(), 
